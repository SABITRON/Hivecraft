...
public class RamLususEntity extends TamableAnimal implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(RamLususEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(RamLususEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(RamLususEntity.class, EntityDataSerializers.STRING);
	...
	public RamLususEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AlterniaModEntities.RAM_LUSUS.get(), world);
	}

	public RamLususEntity(EntityType<RamLususEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "ram_noodle");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(12, new TemptGoal(this, 1, Ingredient.of(AlterniaModItems.SHATTERGRAIN.get()), true));
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		RamLususEntity retval = AlterniaModEntities.RAM_LUSUS.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(AlterniaModItems.SHATTERGRAIN.get()).contains(stack.getItem());
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.ram_noodle.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.ram_noodle.idle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("animation.ram_noodle.bite"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(RamLususEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
