...
public class RenderSkyProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, double partialTick) {
		if (entity == null)
			return;
		if ((entity.level().dimension()) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("alternia:alternia")))) {
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			renderDeepSky(200 << 24 | 35 << 16 | 20 << 8 | 84);
			renderStars(1000, 413, 90, 0, 0, (int) ((world instanceof ClientLevel ? ((ClientLevel) world).getStarBrightness((float) partialTick) : 0.0F) * 255) << 24 | 255 << 16 | 255 << 8 | 255, true);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("alternia" + ":textures/" + "moons" + ".png")));
			renderMoon(40, 255 << 24 | 255 << 16 | 255 << 8 | 255, false, true);
			RenderSystem.setShaderTexture(0, new ResourceLocation(("alternia" + ":textures/" + "sun" + ".png")));
