...
public class RenderPinkMoonProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != RenderPinkMoonProcedure.amount || seed != RenderPinkMoonProcedure.seed) {
			RenderPinkMoonProcedure.amount = amount;
			RenderPinkMoonProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static void execute(@Nullable Event event) {
		RenderSystem.setShaderTexture(0, new ResourceLocation(("alternia" + ":textures/" + "pink_moon.png" + ".png")));
		renderSun(20, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
	}
}
