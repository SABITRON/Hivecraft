...
public class RenderStarsProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != RenderStarsProcedure.amount || seed != RenderStarsProcedure.seed) {
			RenderStarsProcedure.amount = amount;
			RenderStarsProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.level().dimension()) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("alternia:alternia")))) {
			renderStars(1000, 413, 90, 0, 0, 127 << 24 | 255 << 16 | 255 << 8 | 255, true);
		}
	}
}
